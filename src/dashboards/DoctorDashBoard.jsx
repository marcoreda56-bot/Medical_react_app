...
export const DoctorDashBoard = () => {
    ...
    const handleCancelAppointment = async (appId) => {
        const { value: reason } = await Swal.fire({
            ...
            inputValidator: (v) => (!v || !v.trim()) ? 'You must provide a reason for cancellation!' : undefined,
        });
        if (!reason) return;
        try {
            ...
            setAppointments(prev => prev.map(a => a.id === appId ? { ...a, ...updatedData, status: 'Cancelled' } : a));
            Toast.fire({ icon: 'success', title: response.data.message || 'Appointment cancelled and refunded.' });
        } catch (err) {
            console.error(err);
            Toast.fire({ icon: 'error', title: err.response?.data?.error || 'Failed to process cancellation.' });
        }
    };
    ...
    const handleCompleteAppointment = async (e) => {
        ...
        if (!medicalRecord.diagnosis.trim() || !medicalRecord.prescription.trim()) {
            Swal.fire({ icon: 'error', title: 'Required Fields Missing', text: 'Please fill in both diagnosis and prescription.', confirmButtonColor: '#0f172a' });
            return;
        }
        try {
    ...
    const handleDeleteSlot = async (slotId) => {
        try {
            ...
            Toast.fire({ icon: 'error', title: 'Cannot delete an actively booked slot.' });
        }
    };
    ...
    const handleGenerateShift = async (e) => {
        ...
        if (!date || !startTime || !workHours || !duration) {
            Swal.fire({ icon: 'error', title: 'Incomplete Parameters', text: 'Please specify Date, Start Time, Working Hours, and Slot Duration.', confirmButtonColor: '#0f172a' });
            return;
        }
        if (date < todayStr) {
            Swal.fire({ icon: 'error', title: 'Past Date', text: 'Schedule generation must target today or a future date.', confirmButtonColor: '#0f172a' });
            return;
        }
        try {
            const now = new Date();
            ...
            let skippedPast = 0, skippedDup = 0;
            const toCreate = [];

            const isOverlapping = (ns, ne) => slots.some(ex => {
                if (ex.date !== date) return false;
                const [es, ee] = ex.time.split(' - ');
                const esm = es.split(':').map(Number).reduce((h, m) => h * 60 + m);
                const eem = ee.split(':').map(Number).reduce((h, m) => h * 60 + m);
            ...
            while (startMins + slotDur <= endMins) {
                ...
                if (date === todayStr && (h < now.getHours() || (h === now.getHours() && m <= now.getMinutes()))) {
                    startMins += slotDur; skippedPast++; continue;
                }
                ...
                if (isOverlapping(ns, startMins)) { skippedDup++; continue; }
                toCreate.push({ date, day: dayName, time: `${slotStart} - ${slotEnd}`, is_booked: false });
            }

            if (toCreate.length === 0) {
                let msg = 'No new slots generated.';
                if (skippedDup > 0) msg = 'All slots overlap with existing ones.';
                if (skippedPast > 0) msg = 'The time range is in the past.';
                Swal.fire({ icon: 'warning', title: 'No Slots Created', text: msg, confirmButtonColor: '#0f172a' });
            } else {
                await Promise.all(toCreate.map(slot => doctorAPI.addSlot(slot)));
                let msg = `Successfully created ${toCreate.length} slot${toCreate.length > 1 ? 's' : ''}.`;
                if (skippedDup > 0) msg += ` (${skippedDup} overlap(s) skipped)`;
        ...
        <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
            {days.map((item) => {
                ...
                return (
                    ...
                    >
                        <span className={`text-[11px] font-semibold tracking-wide ${isActive ? 'text-slate-400' : 'text-slate-500'}`}>
                            {isToday ? 'Today' : DAY_ABBR[d.getDay()]}
                        </span>
                        <span className={`text-lg font-bold leading-none ${isActive ? 'text-white' : 'text-[#0f172a]'}`}>
                            {d.getDate()}
                        </span>
                        {cnt > 0 ? (
                            <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${isActive ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-600'}`}>
                                {cnt}
                            </span>
                        ) : (
                ...
                <div className="max-w-5xl mx-auto space-y-8">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-7">
                        <div>
                            ...
                            <p className="text-slate-500 text-sm mt-1 font-medium">Manage your schedule, appointments, and patient records.</p>
                        </div>
                        <button
                    ...
                    <div className="flex border-b border-slate-200 gap-7">
                        {['appointments', 'schedule', 'history'].map(tab => (
                            ...
                            >
                                {tab === 'history' ? 'Case Archive' : tab === 'appointments' ? 'Appointments' : 'Schedule'}
                            </button>
                        ))}
                    </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                {[
                                    ...
                                    { label: 'Awaiting Approval',    value: pendingTotal.length },
                                    { label: 'Confirmed Today',       value: confirmedToday.length },
                                ].map(s => (
                            ...
                            <div>
                                ...
                                {appointmentsForDate.length === 0 ? (
                                    <div className="bg-white border border-slate-200/60 rounded-2xl py-16 text-center shadow-sm">
                                        <p className="text-slate-500 text-sm font-semibold">No appointments scheduled for this day.</p>
                                    </div>
                                ) : (
                                                ...
                                                return (
                                                    <div key={app.id} className="bg-white border border-slate-200/60 rounded-2xl px-5 py-4 flex items-center gap-4 shadow-sm hover:border-slate-300 transition-all">
                                                        ...
                                                        <div className="min-w-[72px] text-center">
                                                            <p className="text-base font-bold text-[#0f172a]">{tStart}</p>
                                                            <p className="text-xs text-slate-500 font-medium">{tEnd}</p>
                                                        </div>
                                                        ...
                                                        <div className="flex gap-2 flex-shrink-0">
                                                            {app.status === 'Pending' && (
                                                                <>
                                                                    <button type="button" onClick={() => handleApproveAppointment(app.id)} className="px-3 py-1.5 bg-[#0f172a] text-white font-semibold rounded-xl hover:bg-slate-800 text-xs transition-all cursor-pointer">Accept</button>
                                                                    <button type="button" onClick={() => handleCancelAppointment(app.id)} className="px-3 py-1.5 bg-white text-slate-600 border border-slate-200 font-semibold rounded-xl hover:bg-slate-50 text-xs transition-all cursor-pointer">Decline</button>
                                                                </>
                                                            )}
                                                            {app.status === 'Confirmed' && (
                                                                <>
                                                                    <button type="button" onClick={() => handleOpenCompleteModal(app)} className="px-3 py-1.5 bg-emerald-600 text-white font-semibold rounded-xl hover:bg-emerald-700 text-xs transition-all cursor-pointer">Complete</button>
                                                                    <button type="button" onClick={() => handleCancelAppointment(app.id)} className="px-3 py-1.5 bg-white text-slate-600 border border-slate-200 font-semibold rounded-xl hover:bg-slate-50 text-xs transition-all cursor-pointer">Cancel</button>
                                                                </>
                                                            )}
                                                        </div>
                                                    </div>
                    ...
                    {activeTab === 'schedule' && (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm h-fit space-y-4">
                                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Configure Shift</h3>
                                <form onSubmit={handleGenerateShift} className="space-y-4" noValidate>
                                    <div className="space-y-1">
                                        ...
                                        <input
                                            type="date" min={todayStr} value={shiftForm.date}
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-xs text-slate-600 font-semibold">Working Hours</label>
                                        <select
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-xs text-slate-600 font-semibold">Slot Duration</label>
                                        <select
                            ...
                            <div className="md:col-span-2 space-y-5">
                                ...
                                {scheduleDates.length === 0 ? (
                                    <div className="bg-white border border-slate-200/60 rounded-2xl py-20 text-center shadow-sm">
                                        <p className="text-slate-500 text-sm font-semibold">No slots created yet. Use the form to generate your first shift.</p>
                                    </div>
                                ) : (
                                    <>
                                        <DayStrip
                                        ...
                                        <div>
                                            <div className="flex items-center justify-between mb-3">
                                                <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">
                                                    {fmtDateLabel(selectedScheduleDate)}
                                                </p>
                                                {slotsForScheduleDate.length > 0 && (
                                                    <span className="text-xs text-slate-600 font-semibold">
                                                        {slotsForScheduleDate.filter(s => s.is_booked).length} booked
                                                        {' / '}
                                                        {slotsForScheduleDate.filter(s => !s.is_booked).length} available
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            {slotsForScheduleDate.length === 0 ? (
                                                <div className="bg-white border border-slate-200/60 rounded-2xl py-14 text-center shadow-sm">
                                                    <p className="text-slate-500 text-sm font-semibold">No slots for this day.</p>
                                                </div>
                                            ) : (
                                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                                    {slotsForScheduleDate.map(s => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <div className="flex items-start justify-between">
                                                                    <div>
                                                                        <p className={`text-sm font-bold ${s.is_booked ? 'text-red-600' : 'text-[#0f172a]'}`}>
                                                                            {tStart}
                                                                        </p>
                                                                        <p className="text-xs text-slate-500 font-medium mt-0.5">{tEnd}</p>
                                                                    </div>
                                                                    {s.is_booked ? (
                    ...
                    {activeTab === 'history' && (
                        <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm space-y-6">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Patient Records</h3>
                                <input
                                    type="text"
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-[#fcfcfc] text-slate-500 font-bold text-xs uppercase tracking-wider border-b border-slate-100">
                                            ...
                                            <th className="p-4 text-center pr-6">Records</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                                        {completedAppointments.length === 0 ? (
                                            <tr>
                                                <td colSpan="4" className="text-center py-20 text-slate-500 font-semibold">No completed sessions found.</td>
                                            </tr>
                                        ) : (
                                            completedAppointments.map(app => (
                                                <tr key={app.id} className="hover:bg-[#fafafa] transition-all">
                                                    <td className="p-4 pl-6 font-semibold text-[#0f172a]">{app.patient_name || 'Anonymous'}</td>
                                                    <td className="p-4 text-slate-600 font-medium">
                                                        {app.paid_at
                                                            ? new Date(app.paid_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })
                                                            : 'N/A'}
                                                    </td>
                                                    <td className="p-4 max-w-xs truncate text-slate-600 font-medium">{app.diagnosis || '—'}</td>
                ...
                {openModal && (
                    <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-[2px] flex items-center justify-center z-50 p-4">
                        <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-6 shadow-xl border border-slate-100">
                            <div>
                                <h3 className="text-lg font-bold text-[#0f172a]">Complete Session</h3>
                                <p className="text-xs text-slate-500 font-medium mt-0.5">Patient: {selectedAppointment?.patient_name}</p>
                            </div>
                            <form onSubmit={handleCompleteAppointment} className="space-y-4" noValidate>
                                <div className="space-y-1.5">
                                    <label className="text-xs font-semibold text-slate-600">Diagnosis <span className="text-red-400">*</span></label>
                                    <textarea rows="2" value={medicalRecord.diagnosis} onChange={e => setMedicalRecord({ ...medicalRecord, diagnosis: e.target.value })} className="w-full p-3 border border-slate-200 rounded-xl text-sm focus:ring-1 focus:ring-slate-300 focus:outline-none text-slate-800 transition-all resize-none font-medium placeholder:text-slate-400 placeholder:font-normal" placeholder="Enter clinical findings..." />
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-xs font-semibold text-slate-600">Prescription <span className="text-red-400">*</span></label>
                                    <textarea rows="3" value={medicalRecord.prescription} onChange={e => setMedicalRecord({ ...medicalRecord, prescription: e.target.value })} className="w-full p-3 border border-slate-200 rounded-xl text-sm focus:ring-1 focus:ring-slate-300 focus:outline-none text-slate-800 transition-all resize-none font-medium placeholder:text-slate-400 placeholder:font-normal" placeholder="List medications and dosages..." />
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-xs font-semibold text-slate-600">Doctor Notes <span className="text-slate-400 font-normal">(optional)</span></label>
                                    <textarea rows="2" value={medicalRecord.doctor_notes} onChange={e => setMedicalRecord({ ...medicalRecord, doctor_notes: e.target.value })} className="w-full p-3 border border-slate-200 rounded-xl text-sm focus:ring-1 focus:ring-slate-300 focus:outline-none text-slate-800 transition-all resize-none font-medium placeholder:text-slate-400 placeholder:font-normal" placeholder="Internal notes for follow-up..." />
                                </div>
                                <div className="flex justify-end gap-2 pt-2">
                                    <button type="button" onClick={() => setOpenModal(false)} className="px-4 py-2 bg-slate-50 text-slate-600 font-semibold rounded-xl text-xs hover:bg-slate-100 cursor-pointer transition-all">Cancel</button>
                                    <button type="submit" className="px-4 py-2 bg-[#0f172a] text-white font-bold rounded-xl text-xs hover:bg-slate-800 cursor-pointer transition-all">Save Record</button>
                                </div>
                            </form>
                        </div>
                    </div>
                ...
                {historyModalOpen && selectedHistoryRecord && (
                    <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-[2px] flex items-center justify-center z-50 p-4">
                        <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-5 shadow-xl border border-slate-100">
                            <h3 className="text-base font-bold text-[#0f172a] border-b border-slate-100 pb-3">Consultation Record</h3>
                            <div className="space-y-4 text-sm">
                                <div className="grid grid-cols-2 gap-3 bg-[#fafafa] p-3 rounded-xl border border-slate-100">
                                    <div>
                                        <span className="text-[11px] text-slate-500 block font-bold uppercase tracking-wide mb-1">Patient</span>
                                        <span className="font-semibold text-slate-800">{selectedHistoryRecord.patient_name || 'Anonymous'}</span>
                                    </div>
                                    <div>
                                        <span className="text-[11px] text-slate-500 block font-bold uppercase tracking-wide mb-1">Session Date</span>
                                        <span className="font-semibold text-slate-800">
                                            {selectedHistoryRecord.paid_at
                                                ? new Date(selectedHistoryRecord.paid_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })
                                                : 'N/A'}
                                        </span>
                                    </div>
                                </div>
                                <div className="space-y-1">
                                    <span className="text-xs font-bold text-slate-500 uppercase tracking-wide block">Diagnosis</span>
                                    <p className="bg-slate-50 p-3 rounded-xl border border-slate-200/60 text-slate-700 leading-relaxed text-xs font-medium">
                                        {selectedHistoryRecord.diagnosis || 'No diagnosis recorded.'}
                                    </p>
                                </div>
                                <div className="space-y-1">
                                    <span className="text-xs font-bold text-slate-500 uppercase tracking-wide block">Prescription</span>
                                    <p className="mono bg-emerald-50/40 p-3 rounded-xl border border-emerald-100/60 text-emerald-900 leading-relaxed text-xs whitespace-pre-line">
                                        {selectedHistoryRecord.prescription || 'No prescription recorded.'}
                                    </p>
                                </div>
                                {selectedHistoryRecord.doctor_notes && (
                                    <div className="space-y-1">
                                        <span className="text-xs font-bold text-slate-500 uppercase tracking-wide block">Doctor Notes</span>
