...
import { 
  ...
  Chip, Modal, Tabs, Tab, Card, CardContent, Avatar
} from '@mui/material';
...
export const PatientDashBoard = () => {
  ...
  useEffect(() => {
    ...
    async function loadPatientData() {
      try {
        ...
        setDoctors(usersSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));

        const slotsQuery = query(collection(db, 'doctor_slots'), where('isBooked', '==', false));
        ...
        usersSnapshot.docs.forEach(dDoc => {
          const dSlots = slotsData.filter(s => s.doctorId === dDoc.id);
          if (dSlots.length > 0) {
        ...
        const appsQuery = query(collection(db, 'appointments'), where('patientId', '==', currentUser.uid));
        const appsSnapshot = await getDocs(appsQuery);
        setMyAppointments(appsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
      } catch (err) {
  ...
  const fetchUpdatedData = async () => {
    const slotsQuery = query(collection(db, 'doctor_slots'), where('isBooked', '==', false));
    const slotsSnapshot = await getDocs(slotsQuery);
    setAllSlots(slotsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
  ...
  const handleBookSlot = async (doctor, slot) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire('Booked!', 'Appointment secured successfully.', 'success');
        fetchUpdatedData();
      } catch (err) {
        console.error(err);
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      {activeTab === 0 && (
        <Box>
          ...
          {doctors.length === 0 ? (
            <Typography variant="body1" color="textSecondary">No registered doctors available right now.</Typography>
          ) : (
                    ...
                    <Card elevation={3} sx={{ borderRadius: 3, display: 'flex', flexDirection: 'column', minHeight: '320px' }}>
                      <CardContent>
                            ...
                            {uniqueDays.map((dayName) => (
                              <Chip
                                ...
                                sx={{
                                  ...
                                  borderColor: '#00796b',
                                  fontWeight: 'bold',
                                  '&:hover': { bgcolor: currentSelectedDay === dayName ? '#004d40' : '#e0f2f1' }
                                }}
                                onClick={() => setSelectedDayForDoc({
                                  ...selectedDayForDoc,
                                  [docItem.id]: dayName
                                })}
                              />
                        ...
                        {slotsForSelectedDay.length === 0 ? (
                          <Typography variant="caption" color="textSecondary">Select a day to view slots.</Typography>
                        ) : (
            ...
            <Table>
              ...
              <TableBody>
                {myAppointments.length === 0 ? (
                  <TableRow><TableCell colSpan={4} align="center" sx={{ py: 4, color: '#999' }}>You haven't booked any appointments yet.</TableCell></TableRow>
                  ...
                  <TableRow key={app.id} sx={{ '&:hover': { bgcolor: '#fafafa' } }}>
                    ...
                    <TableCell>
                      {app.status === 'Completed' && app.medicalRecord ? (
                        <Button size="small" variant="contained" sx={{ bgcolor: '#00796b' }} onClick={() => handleOpenRecord(app.medicalRecord)}>
                          View Prescription 📄
                        </Button>
                      ) : (
                        <Typography variant="caption" color="textSecondary">Available after visit</Typography>
                      )}
                    </TableCell>
                  </TableRow>
                ))}
      ...
      <Modal open={openModal} onClose={() => setOpenModal(false)}>
        <Box sx={modalStyle}>
          <Typography variant="h5" sx={{ fontWeight: 'bold', color: '#00796b', mb: 2, textAlign: 'center' }}>
            Official Medical Prescription 🩺
          </Typography>
          <Typography variant="caption" display="block" sx={{ mb: 3, textAlign: 'center', color: '#777' }}>
            Date of Visit: {selectedRecord?.date}
          </Typography>
          
          <Box sx={{ mb: 2, p: 2, bgcolor: '#f9f9f9', borderRadius: 2 }}>
            <Typography variant="subtitle2" sx={{ fontWeight: 'bold', color: '#333' }}>Diagnosis (التشخيص):</Typography>
          ...
          <Box sx={{ mb: 3, p: 2, bgcolor: '#e0f2f1', borderRadius: 2, borderLeft: '4px solid #00796b' }}>
            <Typography variant="subtitle2" sx={{ fontWeight: 'bold', color: '#00796b' }}>Prescription & Notes (العلاج والتعليمات):</Typography>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center' }}>
            <Button variant="contained" sx={{ bgcolor: '#00796b' }} onClick={() => setOpenModal(false)}>Close</Button>
          </Box>
        </Box>
      </Modal>
    </Container>
