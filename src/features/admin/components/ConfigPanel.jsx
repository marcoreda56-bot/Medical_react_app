...
export const ConfigPanel = ({ configs, configForm, onConfigChange, onSubmit, editingConfig, onEdit, onCancel, onDelete }) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-bold text-slate-800 mb-4">{t('admin.panel.configPanel.title')}</h3>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-end bg-slate-50/50 p-4 border border-slate-100 rounded-xl">
          <div className="w-full sm:max-w-xs flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-slate-500">{t('admin.panel.configPanel.configKey')}</label>
            <input
              type="text"
          ...
          <div className="w-full flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-slate-500">{t('admin.panel.configPanel.configValue')}</label>
            <input
              type="text"
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto pt-2 sm:pt-0">
            ...
            >
              <Plus size={16} />
              <span>{editingConfig ? t('admin.panel.configPanel.saveConfig') : t('admin.panel.configPanel.addConfig')}</span>
            </button>
            {editingConfig && (
              <button
                onClick={onCancel}
              ...
              >
                {t('admin.panel.configPanel.cancel')}
              </button>
            )}
          </div>
        </div>
      ...
      <div className="overflow-x-auto border border-slate-100 rounded-xl">
        <table className="w-full text-sm text-left text-slate-600 border-collapse">
          <thead className="text-xs font-semibold text-slate-700 bg-lime-50/60 uppercase border-b border-slate-100">
            <tr>
              <th className="px-6 py-4">{t('admin.panel.configPanel.key')}</th>
              <th className="px-6 py-4">{t('admin.panel.configPanel.value')}</th>
              <th className="px-6 py-4 text-right">{t('admin.panel.configPanel.actions')}</th>
            </tr>
          </thead>
            ...
            {configs.length === 0 ? (
              <tr>
                <td colSpan={3} className="px-6 py-10 text-center text-slate-400 font-medium">
                  {t('admin.panel.configPanel.noConfig')}
                </td>
              </tr>
            ) : (
              configs.map((config) => (
